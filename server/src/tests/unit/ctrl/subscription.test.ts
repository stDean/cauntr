...
const mockUpdateJob = SubscriptionJobs.updateSubscriptionJob as jest.Mock;
const mockCancelJob = SubscriptionJobs.cancelSubscriptionJob as jest.Mock;
...
describe("Subscription Controller", () => {
	...
	describe("updateSubscription", () => {
		it("should update subscription successfully", async () => {
			// Setup mocks
			mockCheckCompany.mockResolvedValue({ company: validCompany as any });
		...
		it("should handle missing company", async () => {
			...
			mockCheckCompany.mockRejectedValue(
				new NotFoundError("Company not found")
			);

			// Execute and verify error
			await expect(
				SubscriptionModule.SubscriptionCtrl.updateSubscription(req, res)
		...
		it("should validate billing parameters", async () => {
			...
			mockCheckBilling.mockRejectedValue(
				new BadRequestError("Billing type and payment plan are required")
			);

			// Execute and verify error
			await expect(
				SubscriptionModule.SubscriptionCtrl.updateSubscription(req, res)
			).rejects.toThrow("Billing type and payment plan are required");
		});
	});
	...
	describe("cancelSubscription", () => {
		...
		it("should handle cancellation job failure", async () => {
			...
			// Execute and verify error
			await expect(
				SubscriptionModule.SubscriptionCtrl.cancelSubscription(req, res)
	...
	describe("reactivateSubscription", () => {
		it("should reactivate subscription successfully", async () => {
			...
			// Execute
			await SubscriptionModule.SubscriptionCtrl.reactivateSubscription(
				req,
				res
			);
		...
		it("should handle Paystack initialization failure", async () => {
			...
			// Execute and verify error
			await expect(
				SubscriptionModule.SubscriptionCtrl.reactivateSubscription(req, res)
			).rejects.toThrow("Payment gateway initialization failed");
		});
