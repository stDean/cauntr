...
const validateCompany = async ({ email, status, tx, }) => {
...
const handleChargeSuccess = async (data, tx) => {
    const { id, customer: { email }, authorization: { authorization_code }, status, } = data;
...
const handleSubscriptionCreate = async (data, tx) => {
    const { subscription_code, next_payment_date, customer: { email }, status, authorization: { authorization_code, last4, exp_month, exp_year, card_type, bank, }, } = data;
    const companyResult = await validateCompany({
        email,
...
const handleInvoiceUpdate = async (data, tx) => {
...
const handleInvoiceFailed = async (data, tx) => {
...
router
    .route("/webhook")
    .post(validatePayStackSignature, async (req, res) => {
    try {
