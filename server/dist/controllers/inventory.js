...
import { generateSKU, handleBuybackProduct, parseDate, productHelper, userNdCompany, } from "../utils/helper.js";
import { productUtils, responseUtils, } from "../utils/helperUtils.js";
...
export const InventoryCtrl = {
    ...
    createProduct: async (req, res) => {
        ...
        if (errors) {
            return responseUtils.error(res, "Validation failed", StatusCodes.BAD_REQUEST, errors);
        }
        ...
        const productData = productService.createProductData(productInput, user, company, supplier);
    ...
    createProducts: async (req, res) => {
        ...
        const productsWithSupplier = req.body.filter((p) => p["Supplier Name"] && String(p["Supplier Phone Number"]));
        // If any products include supplier details, perform a bulk getOrCreate.
        const suppliers = productsWithSupplier.length > 0
            ? await supplierService.bulkGetOrCreate(productsWithSupplier.map((p) => ({
        ...
        const results = [];
        const errors = [];
        await Promise.all(req.body.map(async (product, index) => {
            try {
                ...
                const productData = productService.createProductData(dataInput, user, company, supplier);
     ...
     */
    getProductCountsByTypeAndBrand: async (req, res) => {
     ...
     */
    getProductsByTypeAndBrand: async (req, res) => {
    ...
    getProductBySKU: async (req, res) => {
        const { params: { sku }, user: { email, companyId }, } = req;
    ...
    updateProduct: async (req, res) => {
        const { body, params: { sku }, user: { email, companyId }, } = req;
        ...
        if (updateData.condition) {
            updateData.condition = updateData.condition.toUpperCase();
    ...
    softDeleteProduct: async (req, res) => {
        const { params: { sku }, user: { email, companyId }, body: { deleteQuantity }, } = req;
        ...
        if (quantityToDelete > product.quantity) {
            throw new BadRequestError("Delete quantity exceeds available product quantity..");
        }
     ...
     */
    getSoftDeletedProductsUsingEvents: async (req, res) => {
        const { email, companyId } = req.user;
        const { company } = await userNdCompany({ email, companyId });
        ...
        const softDeletedProducts = products
            .map((product) => {
            const totalDeleted = product.ProductDeletionEvent.reduce((sum, event) => sum + event.quantity, 0);
            if (totalDeleted > 0) {
                const lastDeletionDate = product.ProductDeletionEvent.reduce((latest, event) => {
     ...
     */
    restoreProductQuantity: async (req, res) => {
        const { params: { sku }, user: { email, companyId }, body: { restoreQuantity }, } = req;
        ...
        const totalDeleted = deletionEvents.reduce((sum, event) => sum + event.quantity, 0);
        // Ensure that the restore quantity does not exceed the total deleted quantity.
        if (quantityToRestore > totalDeleted) {
            throw new BadRequestError("Restore quantity exceeds deleted product quantity.");
        }
     ...
     */
    bulkHardDeleteSoftDeletedProducts: async (req, res) => {
        ...
        if (products.length === 0) {
            throw new NotFoundError("No soft-deleted products found for the provided SKUs.");
        }
    ...
    getInventorySummary: async (req, res) => {
        ...
        const totalSellingPrice = products.reduce((sum, product) => sum + product.quantity * Number(product.sellingPrice), 0);
    ...
    createAllBanks: async (req, res) => {
        ...
        if (bank)
            throw new BadRequestError("This bank already exist.");
    ...
    dashboardSummary: async (req, res) => {
        ...
        const [transactions, products, lowStockProducts, outOfStock] = await Promise.all([
        ...
        const months = Array.from({ length: 12 }, (_, i) => new Date(2023, i).toLocaleString("default", { month: "short" }));
        ...
        const topSellingProduct = allTopSellingProducts.length > 0
        ...
        const monthlyData = Object.fromEntries(months.map((m) => [m, { sales: 0, purchases: 0 }]));
        ...
        transactions.forEach((t) => {
            ...
            const cost = t.TransactionItem.reduce((sum, item) => sum + Number(item.Product.costPrice) * Number(item.quantity), 0);
