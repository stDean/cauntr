...
export const InvoiceCtrl = {
    createInvoice: async (req, res) => {
        ...
        if (!company)
            throw new UnauthenticatedError("No company found");
        return prisma.$transaction(async (tx) => {
            ...
            const transaction = await tx.transaction.create({
                data: {
                    ...
                    TransactionItem: {
                        create: body.productDetails.map((product) => {
            ...
            await tx.payment.create({
                data: {
                    ...
                    acctPaidTo: method.toUpperCase() === "CASH"
                        ? undefined
                        : {
                            connectOrCreate: {
    ...
    getInvoice: async (req, res) => {
        const { user, params: { invoiceNo }, } = req;
        ...
        const returnedData = {
            ...
            bankPaidTo: {
                bankName: invoice.Transaction?.Payments[0].payments[0].acctPaidTo?.bank
                    ?.bankName,
                acctNo: invoice.Transaction?.Payments[0].payments[0].acctPaidTo?.bank?.acctNo,
                acctName: invoice.Transaction?.Payments[0].payments[0].acctPaidTo?.bank
                    ?.acctName,
            },
        };
        console.log({
    ...
    markAsPaid: async (req, res) => {
        const { user, params: { invoiceNo, paymentId, planId }, } = req;
        const { company } = await userNdCompany(user);
        if (!company)
            throw new UnauthenticatedError("No company found");
    ...
    recordPayment: async (req, res) => {
        const { user, params: { invoiceNo, planId }, body: { amount, method, paymentMethod }, } = req;
        const { company } = await userNdCompany(user);
        if (!company)
            throw new UnauthenticatedError("No company found");
        return prisma.$transaction(async (tx) => {
            ...
            const payment = await tx.payment.create({
                data: {
                    ...
                    balanceOwed: Number(paymentPlan?.payments[0].balanceOwed) - Number(amount),
                    acctPaidTo: method.toUpperCase() === "CASH"
                        ? undefined
                        : {
                            connectOrCreate: {
            ...
            const updatedInvoice = await tx.invoice.update({
                ...
                data: {
                    paymentDate: new Date(),
                    status: Number(paymentPlan?.payments[0].balanceOwed) - Number(amount) === 0
                        ? "PAID"
                        : "PART_PAID",
                    Transaction: {
                        update: {
                            Payments: {
                                update: {
                                    where: { id: planId },
                                    data: {
                                        customerType: Number(paymentPlan?.payments[0].balanceOwed) -
                                            Number(amount) ===
                                            0
                                            ? "CUSTOMER"
                                            : "DEBTOR",
    ...
    invoiceSummary: async (req, res) => {
        ...
        const customers = Array.from(new Map(invoices
