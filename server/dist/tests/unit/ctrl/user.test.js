...
describe("User Controller", () => {
    ...
    describe("createUser", () => {
        it("should create a new user successfully", async () => {
            const req = mockRequest({ email: "test@example.com", password: "password123" }, {}, { companyId: "1", email: "admin@company.com" });
        ...
        it("should throw error for existing user", async () => {
            const req = mockRequest({ email: "existing@example.com" }, {}, { companyId: "1" });
            const res = mockResponse();
            userNdCompany.mockResolvedValue({
                company: { id: "company1", tenantId: "tenant1" },
            });
            ...
            await expect(UserCtrlModule.UserCtrl.createUser(req, res)).rejects.toThrow("User already exists");
        });
    });
    describe("getUsers", () => {
    ...
    describe("getUser", () => {
        ...
        it("should throw error for non-existent user", async () => {
            ...
            prisma.user.findUnique.mockResolvedValue(null);
            await expect(UserCtrlModule.UserCtrl.getUser(req, res)).rejects.toThrow("User not found");
        });
    });
    describe("updateUserRole", () => {
        it("should update user role to uppercase", async () => {
            ...
            getUserHelper.mockResolvedValue({ user: { id: "user1" } });
            const req = mockRequest({ role: "admin" }, { id: "user1" }, { companyId: "company1", email: "test@example.com" });
            const res = mockResponse();
            prisma.user.update.mockResolvedValue({
                id: "user1",
                role: "ADMIN",
            });
            await UserCtrlModule.UserCtrl.updateUserRole(req, res);
    ...
    describe("updateUserProfile", () => {
        it("should update user profile with password hash", async () => {
            const req = mockRequest({ password: "newpassword", name: "Updated Name" }, { id: "user1" }, { companyId: "1" });
            const res = mockResponse();
            userNdCompany.mockResolvedValue({
                company: { id: "company1", tenantId: "tenant1" },
            });
        ...
        it("should retain old password if new password is empty", async () => {
            const req = mockRequest({ password: "", email: "updated@example.com" }, { id: "user1" }, { companyId: "company1", email: "test@example.com" });
            const res = mockResponse();
            userNdCompany.mockResolvedValue({
                company: { id: "company1", tenantId: "tenant1" },
            });
            getUserHelper.mockResolvedValue({
    ...
    describe("createCustomer", () => {
        it("should create new customer", async () => {
            const req = mockRequest({ name: "Test Customer", phone: "1234567890" }, {}, { companyId: "1" });
            const res = mockResponse();
            userNdCompany.mockResolvedValue({
                company: { id: "company1", tenantId: "tenant1" },
            });
        ...
        it("should throw error if customer already exists", async () => {
            ...
            const req = mockRequest({ name: "John Doe", phone: "1234567890" }, {}, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await expect(UserCtrlModule.UserCtrl.createCustomer(req, res)).rejects.toThrow(BadRequestError);
    ...
    describe("getCustomers", () => {
        it("should return a combined list of customers", async () => {
            ...
            prisma.customer.findMany.mockResolvedValue(createdCustomers);
            const paymentPlanCustomers = [{ Customer: { id: "cust2" } }];
            prisma.paymentPlan.findMany.mockResolvedValue(paymentPlanCustomers);
            const req = mockRequest({}, {}, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await UserCtrlModule.UserCtrl.getCustomers(req, res);
    ...
    describe("getCustomer", () => {
        it("should return a specific customer", async () => {
            ...
            prisma.paymentPlan.findFirst.mockResolvedValue(customer);
            const req = mockRequest({}, { id: "cust1" }, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await UserCtrlModule.UserCtrl.getCustomer(req, res);
    ...
    describe("updateCustomer", () => {
        it("should update an existing customer", async () => {
            ...
            const req = mockRequest({ name: "John Doe", phone: "1234567890", address: "New Address" }, { id: "cust1" }, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            prisma.customer.update.mockResolvedValue({ id: "cust1" });
            await UserCtrlModule.UserCtrl.updateCustomer(req, res);
    ...
    describe("getDebtors", () => {
        it("should return all debtors", async () => {
            ...
            prisma.paymentPlan.findMany.mockResolvedValue(paymentPlans);
            const req = mockRequest({}, {}, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await UserCtrlModule.UserCtrl.getDebtors(req, res);
            expect(prisma.paymentPlan.findMany).toHaveBeenCalledWith({
    ...
    describe("getDebtor", () => {
        it("should return a specific debtor", async () => {
            ...
            const req = mockRequest({}, { id: "cust1" }, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await UserCtrlModule.UserCtrl.getDebtor(req, res);
            expect(prisma.paymentPlan.findFirst).toHaveBeenCalledWith({
    ...
    describe("createSupplier", () => {
        it("should create a new supplier if one doesn't exist", async () => {
            ...
            const req = mockRequest({ name: "Supplier A", contact: "1234567890" }, {}, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await UserCtrlModule.UserCtrl.createSupplier(req, res);
        ...
        it("should throw an error if supplier already exists", async () => {
            ...
            const req = mockRequest({ name: "Supplier A", contact: "1234567890" }, {}, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await expect(UserCtrlModule.UserCtrl.createSupplier(req, res)).rejects.toThrow(BadRequestError);
    ...
    describe("getSuppliers", () => {
        it("should return a list of suppliers", async () => {
            ...
            const req = mockRequest({}, {}, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await UserCtrlModule.UserCtrl.getSuppliers(req, res);
    ...
    describe("getSupplier", () => {
        it("should return a specific supplier", async () => {
            ...
            const req = mockRequest({}, { id: "supplier1" }, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await UserCtrlModule.UserCtrl.getSupplier(req, res);
            expect(prisma.supplier.findUnique).toHaveBeenCalledWith({
    ...
    describe("updateSupplier", () => {
        it("should update an existing supplier", async () => {
            ...
            const req = mockRequest({ name: "Updated Supplier A" }, { id: "supplier1" }, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            const updatedSupplier = { id: "supplier1", name: "Updated Supplier A" };
            prisma.supplier.update.mockResolvedValue(updatedSupplier);
            await UserCtrlModule.UserCtrl.updateSupplier(req, res);
    ...
    describe("deleteSupplier", () => {
        it("should delete supplier if no products exist", async () => {
            ...
            const req = mockRequest({}, { id: "supplier1" }, { companyId: "company1", email: "test@test.com" });
            const res = mockResponse();
            await UserCtrlModule.UserCtrl.deleteSupplier(req, res);
