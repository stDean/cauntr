...
describe("SubscriptionJobs", () => {
    ...
    describe("cancelSubscriptionJob", () => {
        it("should complete cancellation workflow successfully", async () => {
            ...
            paystackService.cancelSubscription.mockResolvedValue({
                error: null,
            });
            // Mock Prisma update
            prisma.company.update.mockResolvedValue({});
            // Mock scheduling
            ScheduleJob.generateCronTime.mockReturnValue("* * * * *");
        ...
        it("should handle Paystack cancellation failure", async () => {
            ...
            await expect(SubscriptionJobs.cancelSubscriptionJob({
                email: mockEmail,
                companyId: mockCompanyId,
                cancelDate: new Date(),
    ...
    describe("updateSubscriptionJob", () => {
        ...
        it("should schedule future update when dates differ", async () => {
            ...
            // Verify scheduling
            expect(scheduleJob).toHaveBeenCalled();
            expect(result).toHaveProperty("msg", "Subscription update scheduled successfully");
        });
    });
    describe("isSameDay", () => {
    ...
    describe("processImmediateUpdate", () => {
        it("should complete immediate update successfully", async () => {
            ...
            expect(paystackService.initializeTransaction).toHaveBeenCalledWith(expect.objectContaining({
                plan: my_plans[`${resBody.paymentPlan.toLowerCase()}_${resBody.billingType.toLowerCase()}`],
                amount: "500000",
                email: mockEmail,
            }));
            // Verify database update
    ...
    describe("scheduleSubscriptionUpdate", () => {
        it("should schedule future updates correctly", async () => {
            ...
            // Mock scheduling
            ScheduleJob.generateCronTime.mockReturnValue("0 0 1 1 *");
            ...
            // Verify scheduling
            expect(scheduleJob).toHaveBeenCalledWith("0 0 1 1 *", expect.any(Function));
        });
    });
});
