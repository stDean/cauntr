...
describe("ScheduleJob", () => {
    ...
    describe("initializeScheduledJobs", () => {
        it("should schedule pending updates and deactivations", async () => {
            ...
            scheduleJob.mockImplementation(() => ({
                name: "test-job",
            }));
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining("Initialized 1 scheduled jobs"));
        });
        it("should handle database errors", async () => {
            prisma.company.findMany.mockRejectedValue(new Error("DB Connection Failed"));
            await expect(ScheduleJob.initializeScheduledJobs()).rejects.toThrow("DB Connection Failed");
            expect(console.error).toHaveBeenCalledWith("Job initialization failed:", expect.any(Error));
        });
    });
    describe("processPendingSubscriptions", () => {
        ...
        it("should handle individual failures gracefully", async () => {
            ...
            expect(console.error).toHaveBeenCalledWith("Failed to process subscription for Error Corp:", expect.any(Error));
        });
    });
    describe("applyPendingSubscription", () => {
        ...
        it("should handle missing pending updates", async () => {
            ...
            await ScheduleJob.applyPendingSubscription("company_123");
            expect(console.log).toHaveBeenCalledWith("No pending subscription update found");
        });
    });
