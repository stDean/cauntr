...
describe("EmailService", () => {
    ...
    describe("sendVerificationOTP", () => {
        ...
        it("should throw error for invalid token length", async () => {
            await expect(emailService.sendVerificationOTP({
                email: testEmail,
                token: invalidToken,
        ...
        it("should generate valid HTML with token spans", async () => {
            ...
            validToken.split("").forEach(char => {
                expect(sentHtml).toContain(`<span style="border: 2px solid #ff5722; padding: 6px 12px; margin: 0 2px; display: inline-block; border-radius: 5px;">${char}</span>`);
            });
        });
