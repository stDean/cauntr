...
export class EmailService {
    ...
    async sendVerificationOTP({ email, token, }) {
      ...
      <div style="width: 700px; margin: auto; font-family: Arial, sans-serif;">
        ...
        <div style="font-size: 24px; font-weight: bold; color: #050201; margin: 20px 0;">
          ${token
            .split("")
            .map((char, index) => `<span style="border: 2px solid #ff5722; padding: 6px 12px; margin: 0 2px; display: inline-block; border-radius: 5px;">${char}</span>`)
            .join("")}
        </div>
    ...
    async sendInvoice(invoiceNo) {
        ...
        const emailContent = generateInvoiceEmail(invoice, // From Prisma query
        companyData, // Your processed company data
        customerData, // Your processed customer data
        res, // Transaction items array
        balances?.[0], bankPaidWith?.[0] // First payment bank details
        );
        await this.sendEmail({
...
`;
const generateInvoiceEmail = (invoice, companyData, customerData, products, paymentData, bankDetails) => `
<div style="width: 700px; margin: auto; font-family: Arial, sans-serif; color: #333; padding: 20px; border: 1px solid #eee;">
  <div style="display: flex; justify-content: space-between; margin-bottom: 30px;">
    <div>
      ...
      <p style="color: ${invoice.status === "PAID" ? "#5cb85c" : "#d9534f"}; margin: 5px 0 0;">
        ${invoice.status === "DRAFT" ? "Pending" : invoice.status}
      </p>
    </div>
    <div style="text-align: right;margin-right: 3px;">
      <p style="margin: 3px 0;">Invoice Date: ${new Date(invoice.createdAt).toLocaleDateString()}</p>
      <p style="margin: 3px 0;">Payment Date: ${new Date(invoice.paymentDate).toLocaleDateString()}</p>
    </div>
  </div>
