...
export class SubscriptionJobs {
     ...
     */
    static async cancelSubscriptionJob({ email, companyId, cancelDate, }) {
        try {
            ...
            console.error(`CancelSubscriptionJob failed for company ${companyId}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    static async scheduleDeactivationJob({ companyId, deactivationDate, }) {
        ...
        console.log(`Scheduled deactivation for ${companyId} at ${deactivationDate.toISOString()}`);
    }
    /**
     ...
     */
    static async updateSubscriptionJob({ billingType, paymentPlan, email, nextBillingDate, companyId, customerId, }) {
        const { error } = await paystackService.cancelSubscription({ email });
        if (error) {
            throw new NotFoundError(error);
        }
        ...
        await paystackService.createSubscription({
            plan: my_plans[`${paymentPlan.toLowerCase()}_${billingType.toLowerCase()}`],
            customer: customerId,
     ...
     */
    static async processImmediateUpdate({ paymentPlan, billingType, email, }) {
     ...
     */
    static async scheduleSubscriptionUpdate({ executeAt, companyId, paymentPlan, billingType, }) {
        ...
        console.log(`Scheduled update for ${companyId} at ${executeAt.toISOString()}`);
    }
    /**
