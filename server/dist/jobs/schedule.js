        ...
        try {
            ...
            for (const company of companies) {
                ...
                if (company.pendingPlanUpdate && company.nextBillingDate) {
                    ...
                    const timeInCronEx = executeAt.getMinutes() +
                        " " +
                    ...
                    console.log(`Scheduled plan update for ${company.company_name} at ${executeAt.toISOString()}`);
                }
                // Handle deactivations
                if (company.scheduledDeactivation) {
                    // Parse as UTC explicitly
                    const deactivationDate = company.scheduledDeactivation;
                    const timeInCronEx = deactivationDate.getMinutes() +
                        " " +
                    ...
                    console.log(`Scheduled deactivation for ${company.company_name} at ${deactivationDate.toISOString()}`);
                }
            }
    ...
    static async processPendingSubscriptions() {
        try {
            ...
            for (const company of pendingCompanies) {
                try {
                    await this.applyPendingSubscription(company.id);
                    console.log(`Processed pending subscription for ${company.company_name}`);
                }
                catch (error) {
                    console.error(`Failed to process subscription for ${company.company_name}:`, error);
